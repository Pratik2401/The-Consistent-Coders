...
export const Jobs = () => {
            ...
            ctx = gsap.context(() => {
                ...
                if (titleElements.length > 0) {
                    titleElements.forEach((el) => {
                        const split = createSplitType(el, { types: 'words' });
                        if (split) {
                ...
                // Reveal up
                const revealUpElements = gsap.utils.toArray('.section-jobs .reveal-up');
                ...
                if (jobCards.length > 0) {
                    gsap.fromTo(jobCards, { y: 50, opacity: 0 }, {
                        y: 0,
                        opacity: 1,
                        duration: 0.8,
                        stagger: 0.1,
    ...
    return (<section id="jobs" className="section-jobs theme-black" ref={sectionRef}>
      ...
      <div className="jobs-grid">
        {jobsData.map((job) => (<div key={job.id} className="job-card reveal-fade" id={job.id}>
            <div className="job-card-inner">
              ...
              <div className="job-skills">
                {job.skills.map((skill, index) => (<span key={index} className="skill-tag">
                    {skill}
                  </span>))}
              </div>
